...
// PR Marmot patch: an uninhabited enum as an extern static's type is a
// future hard error (rust-lang/rust#74840). An opaque, zero-sized repr(C)
// struct is the standard FFI stand-in; `*const Class` is unchanged.
#[repr(C)]
struct Class {
    _private: [u8; 0],
}
